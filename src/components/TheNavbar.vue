  ...
  <div>
    <header class="absolute top-0 left-0 right-0 z-10">
      <nav class="flex justify-between bg-transparent">
        ...
        <div class="flex">
          <button
            class="bg-informative text-white px-[40px] py-[29px] rounded-bl-lg hidden md:block"
          >
            PSB Online
          </button>
          <button
      ...
      <!-- Menu Slide Down -->
      <div
        class="fixed top-0 left-0 w-full transform transition-transform duration-300 z-50 flex flex-col"
        :class="isSidebarOpen ? 'translate-y-0' : '-translate-y-full'"
      >
        <!-- Navbar Slidedown-->
        <nav class="w-full flex justify-between bg-white text-white">
          <!-- Logo -->
          <div class="w-[500px] h-full bg-[#02060d]">
            <div class="flex items-center p-5 pb-2 md:px-[80px] md:pt-[29px]">
              <img
                src="@/assets/img/logo.png"
                alt="Logo"
        ...
        <!-- Sidebar Content -->
        <div class="flex w-full max-h-[500px] h-[500px] overflow-hidden">
          <!-- Main Menu Left -->
          <aside class="w-[500px] h-full bg-[#02060d] text-white flex flex-col">
            <!-- <div class="flex items-center p-5 pb-2 md:px-[80px] md:pt-[29px]">
                            <img src="@/assets/img/logo.png" alt="Logo" class="w-[67px] h-full md:w-[103px] md:h-[56px] object-contain" />
                        </div> -->
            <ul class="flex flex-col gap-6 px-[60px] pt-8 text-3xl text-[#667085] mt-8">
              <li><button @click="activeMenu = 'Profil'">Profil</button></li>
              <li><button @click="activeMenu = 'Akademik'">Akademik</button></li>
              <li><button @click="activeMenu = 'Program'">Program</button></li>
              <li><button @click="activeMenu = 'Fasilitas'">Fasilitas</button></li>
              <li><button @click="activeMenu = 'Berita'">Berita</button></li>
            </ul>

            <!-- Sosmed -->
            <div class="px-[60px] mt-auto mb-10 flex gap-4">
              <!-- Facebook -->
              <a
                href="https://facebook.com"
                target="_blank"
                aria-label="Facebook"
                class="w-[48px] h-[48px] flex items-center justify-center border border-white rounded-full text-white hover:bg-white/10 transition"
              >
                <svg
                  width="21"
                  height="20"
          ...
          <div class="flex-1 bg-white p-8">
            <div class="max-w-[400px]">
              <!-- Konten Profil -->
              <div v-if="activeMenu === 'Profil'" class="flex flex-col gap-[48px]">
                <div class="flex flex-col gap-4">
                  <h2 class="text-2xl font-bold">Tentang DIMSA</h2>
                  <p>
                    Telusuri lebih dekat DIMSA dalam sejarah, rekam kontribusi untuk negeri dan
                    beragam informasi lainnya.
                  </p>
                </div>

                <ul class="flex flex-col gap-6 text-xl font-bold">
                  <li>
                    <router-link to="/selayang-pandang" @click="isSidebarOpen = false"
                      >Selayang Pandang</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/sejarah" @click="isSidebarOpen = false">Sejarah</router-link>
                  </li>
                  <li>
                    <router-link to="/visi-misi" @click="isSidebarOpen = false"
                      >Visi & Misi</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/struktur-organisasi" @click="isSidebarOpen = false"
                      >Struktur Oranisasi</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/akreditas" @click="isSidebarOpen = false"
                      >Akreditasi</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/pimpinan" @click="isSidebarOpen = false"
                      >Pimpinan & Dewan Guru</router-link
                    >
                  </li>
                </ul>
              </div>
              ...
              <!-- Konten Akademik -->
              <div v-else-if="activeMenu === 'Akademik'" class="flex flex-col gap-[40px]">
                <div class="flex flex-col gap-4">
                  <h2 class="text-2xl font-bold">Akademik</h2>
                  <p class="text-base text-[#8796A1] font-normal">
                    Ragam informasi akademik lembaga pendidikan di DIMSA
                  </p>
                </div>

                <ul class="flex flex-col gap-6 text-xl font-bold">
                  <li>
                    <router-link to="/akademik/smp" @click="isSidebarOpen = false">SMP</router-link>
                  </li>
                  <li>
                    <router-link to="/akademik/sma" @click="isSidebarOpen = false">SMA</router-link>
                  </li>
                </ul>
              </div>

              <!-- Konten Program -->
              <div v-else-if="activeMenu === 'Program'" class="flex flex-col gap-[40px]">
                <div class="flex flex-col gap-4">
                  <h2 class="text-2xl font-bold">Program</h2>
                  <p class="text-base text-[#8796A1] font-normal">
                    Ragam pembelajaran unggulan DIMSA untuk pengembangan diri Siswa-siswi
                  </p>
                </div>

                <ul class="flex flex-col gap-6 text-xl font-bold">
                  <li>
                    <router-link to="/program-kurikulum" @click="isSidebarOpen = false"
                      >Kurikulum Pondok</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/program-tahfidz" @click="isSidebarOpen = false"
                      >Program Tahfidz</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/program-bahasa" @click="isSidebarOpen = false"
                      >Program Bahasa</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/ekstra" @click="isSidebarOpen = false"
              ...
              <!-- Konten Fasilitas -->
              <div v-else-if="activeMenu === 'Fasilitas'" class="flex flex-col gap-[40px]">
                <div class="flex flex-col gap-4">
                  <h2 class="text-2xl font-bold">Fasilitas</h2>
                  <p class="text-base text-[#8796A1] font-normal">
                    Pendukung efektifitas proses kegiatan pembelajaran di DIMSA
                  </p>
                </div>

                <ul class="flex flex-col gap-6 text-xl font-bold">
                  <li>
                    <router-link to="/sarana" @click="isSidebarOpen = false"
                      >Sarana prasarana</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/tata-tertib" @click="isSidebarOpen = false"
                      >Panduan Tata Tertib</router-link
                    >
                  </li>
                </ul>
              </div>
              ...
              <!-- Konten Berita -->
              <div v-else-if="activeMenu === 'Berita'" class="flex flex-col gap-[40px]">
                <div class="flex flex-col gap-4">
                  <h2 class="text-2xl font-bold">Berita</h2>
                  <p class="text-base text-[#8796A1] font-normal">
                    Kumpulan informasi uptodate DIMSA
                  </p>
                </div>

                <ul class="flex flex-col gap-6 text-xl font-bold">
                  <li>
                    <router-link to="/detail-berita/:slug" @click="isSidebarOpen = false"
                      >Dimsa dalam Berita</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/majalah" @click="isSidebarOpen = false">Majalah</router-link>
                  </li>
                  <li>
                    <router-link to="/galeri-berita" @click="isSidebarOpen = false"
                      >Galeri</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/pengumuman" @click="isSidebarOpen = false"
                      >Pengumuman</router-link
                    >
                  </li>
                  <li><router-link to="/qna" @click="isSidebarOpen = false">QnA</router-link></li>
                  <li>
                    <router-link to="/alumni" @click="isSidebarOpen = false"
                      >Jejaring Alumni</router-link
                    >
                  </li>
                  <li>
                    <router-link to="/lowongan" @click="isSidebarOpen = false"
