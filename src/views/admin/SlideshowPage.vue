  ...
  <div class="p-6 space-y-6">
    ...
    <div class="bg-white shadow rounded p-4">
      ...
      <div class="mt-4 space-x-2">
        <button
          type="button"
          @click.stop="isEdit ? updateSlide() : createSlide()"
          :disabled="
            loading || (!isEdit && (!form.headline || !form.file)) || (isEdit && !form.headline)
          "
...
export default {
  ...
  methods: {
    ...
    // CREATE
    async createSlide() {
      if (!this.form.headline || !this.form.file) {
        alert('Mohon isi headline dan pilih gambar terlebih dahulu.')
        return
      }
      ...
      try {
        ...
        alert(e?.response?.data?.message || 'Gagal menyimpan slideshow')
      } finally {
        this.loading = false
      }
    },
    ...
    async updateSlide() {
      if (!this.form.headline) {
        alert('Mohon isi headline terlebih dahulu.')
        return
      }
      const formData = new FormData()
      formData.append('headline', this.form.headline)
      ...
      try {
        ...
        alert(e?.response?.data?.message || 'Gagal memperbarui slideshow')
      } finally {
        this.loading = false
      }
    },
